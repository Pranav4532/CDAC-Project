...
const AdminAddEditBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const bookPayload = { ...bookData, imageUrl: uploadedImageUrl };
  ...
  return (
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-danger">{isEditMode ? "Update Book" : "Add Book"}</button>
        <button type="button" className="btn btn-secondary ms-2" onClick={() => navigate("/bookmanagement")}>
          Cancel
        </button>
      </form>
    </div>
...
export default AdminAddEditBook;
