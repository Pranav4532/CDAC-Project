...
const BookDetail = () => {
  ...
  const fetchSimilarBooks = async (title) => {
    ...
    try {
      ...
      console.log("Fetched Similar Books:", books.map((b) => b.id)); // Debugging log
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <ScrollView style={styles.container}>
        ...
        <TouchableOpacity style={styles.button} onPress={() => toggleWishlist(book)}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.similarBooksContainer}>
          {similarBooks.length > 0 ? (
            similarBooks.map((item, index) => {
              const bookId = item.id || `book-${index}`; // Ensure unique key
              return (
                <TouchableOpacity
                  key={bookId}
